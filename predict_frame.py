...
from TrackNetv2.predict_3_in_1_out.TrackNet import TrackNet
...
def get_ball_position(videoName : str):
    ...
    start = time.time()
    modelName = "TrackNetv2/predict_3_in_1_out/model_33"
...
for video in video_list:
    videoName = "src_videos/" + video + ".mp4"
    answerCSV = f"part1/train/{video}/{video}_S2.csv"
    ans = pd.read_csv(answerCSV)
    ans = ans[ans.columns[1]]
    ball_position = get_ball_position(videoName)
    ...
    print(f"Predicted Hit Frame = {predict_output}, Hit Number = {len(predict_output)}")
    print("============================================我是分隔線============================================\n")
